...
(*let rec send_msg sending oc =
  begin match sending with
  | true -> 
    let msg = input_line Stdlib.stdin in output_string oc (msg^"\n"); 
    if msg = "" then print_endline "empty line! \n";
    if msg = "" then (send_msg false oc) else (send_msg true oc) 
  | false -> ()
  end*)
...
(*let rec send_msg oc msg =
  ...
  | "" -> ()
  | _ -> 
    let new_msg = input_line Stdlib.stdin 
    in output_string oc (new_msg^"\n"); 
    send_msg oc new_msg
  end*)
